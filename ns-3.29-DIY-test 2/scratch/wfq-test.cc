...
#include "ns3/tcp-multcp.h"
#include "ns3/multcp-tag.h"
...
NS_LOG_COMPONENT_DEFINE ("WfqTest");
...
uint32_t switchBufferLength = 200;
...
TxSocketCreateTrace (double weight, uint32_t flowId, Ptr<Socket> socket)
{
  ...
  NS_LOG_UNCOND ("Create Tx Socket ----" << " Type: " << "TcpNewReno" << " Weight: " << weight << " Flow Id: " << flowId);
}
  ...
  for (uint32_t i = 0; i < linkNum; ++i)
    {
      ...
      p2p.SetChannelAttribute ("Delay", StringValue (linkDelay));
          ...
          QueueDiscContainer hostQueue;
					...
					hostQueue = tch1.Install (devices.Get (0));

          //install Switch Queue
					QueueDiscContainer switchQueue;
      ...
      else if (qdiscType == "Wfq")
        {
          //install host queue
          QueueDiscContainer hostQueue;
					TrafficControlHelper tch1;
					tch1.SetRootQueueDisc ("ns3::WfqQueueDisc", "MaxSize", QueueSizeValue (QueueSize(ns3::QueueSizeUnit::PACKETS, qdiscSize)));
					hostQueue = tch1.Install (devices.Get (0));

          //install Switch Queue
					QueueDiscContainer switchQueue;
          TrafficControlHelper tch2;
					tch2.SetRootQueueDisc ("ns3::WfqQueueDisc", "MaxSize", QueueSizeValue (QueueSize(ns3::QueueSizeUnit::PACKETS, qdiscSize)));
					switchQueue = tch2.Install (devices.Get (1));
...
main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("switchBufferSize", "The size of switch queue", switchBufferLength);
