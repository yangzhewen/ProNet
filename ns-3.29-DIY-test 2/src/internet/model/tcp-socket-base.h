...
public:
  ...
  /**
   * \brief Callback pointer for raw RTT trace chaining
   */
  TracedCallback<Time, Time> m_rawRttTrace;
  ...
  /**
   * \brief Callback function to hook to TcpSocketState raw rtt
   * \param oldValue old raw rtt value
   * \param newValue new raw rtt value
   */
  void UpdateRawRtt (Time oldValue, Time newValue);
...
protected:
  ...
  virtual void SetTenantId (uint tenantId);
  virtual uint GetTenantId (void) const;
