...
void TcpSocketBase::SetWeight (double w) {
  m_tcb->m_weight = w; 
}
double TcpSocketBase::GetWeight (void) const { return m_tcb->m_weight; }
void TcpSocketBase::SetTenantId (uint tenantId) { m_tcb->m_tenantId = tenantId; }
uint TcpSocketBase::GetTenantId (void) const { return m_tcb->m_tenantId; }
...
//                   MakeEnumChecker (CLOSED, "Closed"))
    ...
    .AddAttribute ("FlowWeight", "Weight of the flow held by the socket",
                   DoubleValue (0.0),
                   MakeDoubleAccessor (&TcpSocketBase::SetWeight,
                                       &TcpSocketBase::GetWeight),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("TenantId", "The id of the tenant that the socket belongs to",
                   UintegerValue (0),
                   MakeUintegerAccessor (&TcpSocketBase::SetTenantId,
                                         &TcpSocketBase::GetTenantId),
                   MakeUintegerChecker<uint32_t> ())                   
    .AddTraceSource ("RTO",
    ...
    .AddTraceSource ("RTT",
                     ...
                     "ns3::TracedValueCallback::Time")
    .AddTraceSource ("RawRTT",
                     "Raw RTT extracted from packets' headers",
                     MakeTraceSourceAccessor (&TcpSocketBase::m_rawRttTrace),
                     "ns3::TracedValueCallback::Time")
  ...
  : TcpSocket ()
{
  ...
  ok = m_tcb->TraceConnectWithoutContext ("RawRTT",
                                          MakeCallback (&TcpSocketBase::UpdateRawRtt, this));
  NS_ASSERT (ok == true);
}
...
void
TcpSocketBase::UpdateRawRtt (Time oldValue, Time newValue)
{
  m_rawRttTrace (oldValue, newValue);
}
